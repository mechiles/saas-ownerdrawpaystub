    ...
    @else
        ...
        @if (env('CASHIER_VENDOR') == 'stripe')
            <div class="flex flex-col">
                <h5 class="mb-2 text-xl font-semibold text-gray-700">Your active subscription is: <span class="font-normal underline">{{ auth()->user()->subscription->plan->name }}</span></h5>
        ...
        @else
            @if(auth()->user()->subscriber())
                ...
                @else
                    <div class="flex flex-col">
                        <h5 class="mb-2 text-xl font-semibold text-gray-700">Your active subscription is: <span class="font-normal">{{ auth()->user()->subscription->plan->name }}</span></h5>
                        {{-- <p class="text-wave-700">You are currently on a free plan or your plan is in trial mode</p> --}}
                    </div>
                @endif
